...
class Workers extends MX_Controller
{
    ...
    function save_data()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if (!empty($_POST['id'])) {
                $where['id'] = $_POST['id'];
                echo $this->mdl_workers->update_data($where, $data);
            } else {
                echo $this->mdl_workers->add_data($data);
            }
        } else {
    ...
    function delete_data()
    {
        if (isset($_GET['id']) && $_GET['id']) {
            ...
            $worker_data = $this->mdl_workers->view_worker($_GET['id']);
            ...
            $where['id'] = $_GET['id'];
            echo $this->mdl_workers->delete_data($where);
        } else
            echo "Not Deleted";
    }
    ...
    function remove_image($title)
    {
        if (substr($title, 0, 4) != "http") {
            $path1 = "./assets/uploads/workers/" . $title;
            unlink($path1);
            $path2 = "./assets/uploads/workers/thumb/" . $title;
            unlink($path2);
        }
    }
}
